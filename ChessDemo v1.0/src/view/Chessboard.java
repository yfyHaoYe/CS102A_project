...
public class Chessboard extends JComponent {
    ...
    public void resetChessboard() {
        ...
        chessGameFrame.setLabelText("current player: " + currentColor);
        initiateEmptyChessboard();
        initChessPiece();
        repaint();
    }
    ...
    public void swapChessComponents(ChessComponent chess1, ChessComponent chess2) {
        ...
        if (!(chess2 instanceof EmptySlotComponent)) {
            ...
            remove(chess2);
            add(chess2 = new EmptySlotComponent(chess2.getChessboardPoint(), chess2.getLocation(), clickController, chessSize));
        }
        ...
        if (end){
            ...
            resetBtn.addActionListener((e) -> {dialog.setVisible(false);resetChessboard();});
            dialog.add(label);
            dialog.add(exitBtn);
            dialog.add(resetBtn);
    ...
    public void initiateEmptyChessboard() {
        for (int i = 0; i < chessComponents.length; i++) {
            for (int j = 0; j < chessComponents[i].length; j++) {
                putChessOnBoard(new EmptySlotComponent(new ChessboardPoint(i, j), calculatePoint(i, j), clickController, chessSize));
            }
        }
    }
    ...
    public void swapColor() {
        ...
        chessGameFrame.setLabelText("current player: " + currentColor); //Task1-1：显示黑白方回合状态
    }

    public void initComponentOnBoard(int kind,int row, int col, ChessColor color) {
        ChessComponent chessComponent = switch (kind) {
            case 1 ->
                    new RookChessComponent(new ChessboardPoint(row, col), calculatePoint(row, col), color, clickController, chessSize);
            case 2 ->
                    new KingChessComponent(new ChessboardPoint(row, col), calculatePoint(row, col), color, clickController, chessSize);
            case 3 ->
                    new QueenChessComponent(new ChessboardPoint(row, col), calculatePoint(row, col), color, clickController, chessSize);
            case 4 ->
                    new BishopChessComponent(new ChessboardPoint(row, col), calculatePoint(row, col), color, clickController, chessSize);
            case 5 ->
                    new KnightChessComponent(new ChessboardPoint(row, col), calculatePoint(row, col), color, clickController, chessSize);
            case 6 ->
                    new PawnChessComponent(new ChessboardPoint(row, col), calculatePoint(row, col), color, clickController, chessSize);
            default ->
                    new EmptySlotComponent(new ChessboardPoint(row,col), calculatePoint(row, col), clickController, chessSize);
        };
    ...
    public void resizedAfter() {
        ...
        for (Component item : components) {
            ...
            c.setLocation(calculatePoint(p.getX(), p.getY()));
